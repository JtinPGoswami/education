...
import { Admissions, CollegeInfo, Courses, Fees, Placements, TextReview, VideoReview } from "../components/CollegeInformations";
...
const CollegeDetailPage = () => {
  ...
  const { selectedCollege } = useCollege();
    if (!selectedCollege) {
      return <div className="text-center py-4 text-gray-600">Loading college details...</div>;
    }
    

  const { placementInfo, admissionInfo, feesInfo, detailsHighlight, courseInfo, FAQ, Img,textReview,videoReview } = selectedCollege;
    ...
    <>
      ...
      <section className="px-4">
        <div className="flex md:gap-8 flex-wrap gap-3 py-4 mt-6 px-4 border border-gray-300 rounded-xl">
          {["College Info", "Courses", "Fees", "Admissions", "Placements", "Reviews"].map((tab) => (
            <button
              key={tab}
              className={`text-gray-600 hover:text-green-500 cursor-pointer font-medium ${activeTab === tab ? "text-green-500" : ""}`}
